...
(function() {
    ...
    function renderToolInterface(toolId) {
        ...
        // ---------- PROFESSIONAL: Markdown → PDF (Academic + TOC + Numbering) ----------
if (toolId === 'md2pdf') {
    console.log('✅ Professional Markdown tool loaded');
    ...
    area.innerHTML = `
        ...
        <button id="printMdBtn" class="secondary">
            🖨️ Print / Save as PDF
        </button>
    `;
    ...
    // -------------------- SECTION NUMBERING --------------------
    function addSectionNumbers(container) {
        let h1 = 0, h2 = 0, h3 = 0;
    ...
    mdFile.addEventListener('change', async () => {
        ...
        mdRendered.innerHTML =
            printStyles(themeSelect.value) +
            `<div class="markdown-body">${html}</div>`;

        addSectionNumbers(mdRendered);
        generateTOC(mdRendered);
